...
{
    public:
        UMFPACKSolver umfpackSolver_;
        RocSPARSESolver rocsparseSolver_;
        ConversionData convData_;
        double rel_err;
        std::vector<double> Dz;

        ErrorReport(UMFPACKSolver& umfpackSolver, RocSPARSESolver& rocsparseSolver, ConversionData& convData)
        :umfpackSolver_(umfpackSolver),
        rocsparseSolver_(rocsparseSolver),
        convData_(convData)
        {
            printf("--- Relative error h_z1 and z1 --- \n");
            rel_err = relativeErrorInfinityNorm(rocsparseSolver_.h_z1, umfpackSolver_.z1);
            printf("rel_error = %f \n", rel_err);

            printf("--- Relative error h_z2 and z2 --- \n");
            rel_err = relativeErrorInfinityNorm(rocsparseSolver_.h_z2, umfpackSolver_.z2);
            printf("rel_error = %f \n", rel_err);

            printf("--- Relative error h_y and y --- \n");
            rel_err = relativeErrorInfinityNorm(rocsparseSolver_.h_y, umfpackSolver_.y);
            printf("rel_error = %f \n", rel_err);

            Dz.resize(umfpackSolver_.z1.size());

            printf("--- Infinity norm of residual of D_w z = B_w x (RocSPARSE) --- \n");
            spmv_csr(convData_.csrDvals, convData_.csrDcols, convData_.csrDrows, rocsparseSolver_.h_z2, Dz, convData_.csrDrows.size() - 1);
            rel_err = relativeErrorInfinityNorm(Dz, rocsparseSolver_.h_z1);
            printf("|Dz-Bx| = %f \n", rel_err);

            printf("--- Infinity norm of residual of D_w z = B_w x (UMFPACK) --- \n");
            spmv_csr(convData_.csrDvals, convData_.csrDcols, convData_.csrDrows, umfpackSolver_.z2, Dz, convData_.csrDrows.size() - 1);
            rel_err = relativeErrorInfinityNorm(Dz, umfpackSolver_.z1);
            printf("|Dz-Bx| = %f \n", rel_err);
        }
};
